...
if uploaded_file:
    ...
    # 📡 **Radar Chart**
    st.subheader("📡 Prediction Comparison (Radar Chart)")
    fig, ax = plt.subplots(figsize=(6, 6), subplot_kw=dict(polar=True))
    categories = list(class_labels.values())
    values = class_probs.tolist()
    values += values[:1]
    ...
    # 🎻 **Violin Plot**
    st.subheader("🎻 Prediction Distribution (Violin Plot)")
    fig, ax = plt.subplots(figsize=(8, 4))
    sns.violinplot(data=[class_probs], inner="stick", palette="coolwarm", ax=ax)
    ax.set_xticks(range(len(class_labels)))
    ax.set_xticklabels(class_labels.values(), rotation=15, ha="right")
    ax.set_ylabel("Confidence Score")
    ...
    sns.heatmap([class_probs], annot=True, fmt=".2f", cmap="coolwarm", xticklabels=class_labels.values(), yticklabels=["Probability"])
    st.pyplot(fig)
